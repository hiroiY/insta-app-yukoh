...
class CategoriesController extends Controller
{
    ...
    public function index() {
        ...
        // SELECT * FROM ategories ORDER BY updated_at DESC

        $uncategorized_count = 0;
        $all_posts = $this->post->all();

        foreach($all_posts as $post) {
            if($post->categoryPost->count() == 0) { //If this return , then the post don't have category
    ...
    public function destroy($id) {
        $category = $this->category->findOrFail($id); //$this->category->destroy($id);
        $category->delete();
